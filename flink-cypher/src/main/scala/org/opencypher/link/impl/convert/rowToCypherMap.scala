...
final case class rowToCypherMap(exprToColumn: Seq[(Expr, String)], columnNameToIndex: Map[String, Int]) extends (Row => CypherMap) {
  ...
  private def collectNode(row: Row, v: Var): CypherValue = {
    ...
    idValue match {
      ...
      case invalidID => throw UnsupportedOperationException(s"LinkNode ID has to be a Long instead of ${invalidID.getClass}")
    }
  }
  ...
  private def collectRel(row: Row, v: Var): CypherValue = {
    ...
    idValue match {
      ...
      case id: Long =>
        ...
        LinkRelationship(id, source, target, relType, properties)
      case invalidID => throw UnsupportedOperationException(s"LinkRelationship ID has to be a Long instead of ${invalidID.getClass}")
    }
  }
