...
trait ElementTableCreationSupport {
  ...
  /**
    ...
    */
  def constructElementTable(pattern: Pattern, table: Table)(implicit session: LinkSession): LinkElementTable = {
    val mapping = pattern.elements.foldLeft(ElementMapping.empty(pattern)) {
      case (acc, patternElement) =>

        val patternElementColumns = table
          .columns
    ...
    LinkElementTable.create(mapping, table)
  }
}
